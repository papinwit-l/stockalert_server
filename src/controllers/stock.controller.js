...
const getWatchlist = async (req, res) => {
  try {
    ...
    res.json(watchlist.map((w) => w.stock));
  } catch (error) {
...
const watchStock = async (req, res) => {
  try {
    ...
    let stock = await prisma.stock.findUnique({ where: { symbol: upperSymbol } });
    ...
    await prisma.watchlist.create({ data: { userId: req.userId, stockId: stock.id } });
...
const unwatchStock = async (req, res) => {
  try {
    ...
    await prisma.watchlist.deleteMany({
      where: { userId: req.userId, stockId: stock.id },
    });
...
module.exports = { getWatchlist, watchStock, unwatchStock, getQuote };
